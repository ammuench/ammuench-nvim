...
function M.numbercolumn(opts)
    ...
    return function(self)
        ...
        local signs = vim.opt.signcolumn:get():find "nu" and
                          vim.fn.sign_getplaced(self.bufnr or vim.api.nvim_get_current_buf(), {
                group = "*",
                lnum = lnum
            })[1].signs
        local str
    ...
    return function() -- move to M.fold_indicator
        ...
        if width ~= 0 then
            ...
            if foldinfo.level == 0 then
                ...
                for col = 1, width do
                    str = str ..
                              ((vim.v.virtnum ~= 0 and foldsep) or
                                  ((closed and (col == foldinfo.level or col == width)) and foldclosed) or
                                  ((foldinfo.start == vim.v.lnum and first_level + col > foldinfo.llevel) and foldopen) or
                                  foldsep)
...
function M.search_count(opts)
    ...
    return function()
        ...
        if search_ok and type(search) == "table" and search.total then
            return status_utils.stylize(string.format("%s%d/%s%d", search.current > search.maxcount and ">" or "",
                math.min(search.current, search.maxcount), search.incomplete == 2 and ">" or "",
                math.min(search.total, search.maxcount)), opts)
        end
    end
end
...
function M.unique_path(opts)
    ...
    return function(self)
        ...
        return status_utils.stylize((opts.max_length > 0 and #unique_path > opts.max_length and
                                        string.sub(unique_path, 1, opts.max_length - 2) .. get_icon "Ellipsis" .. "/") or
                                        unique_path, opts)
    end
end
...
function M.file_icon(opts)
    return function(self)
        ...
        if not ft_icon then
            ft_icon, _ = devicons.get_icon_by_filetype(vim.bo[bufnr].filetype, {
                default = true
            })
        end
...
function M.git_diff(opts)
    ...
    return function(self)
        ...
        return status_utils.stylize(status and status[opts.type] and status[opts.type] > 0 and
...
function M.lsp_progress(opts)
    ...
    return function()
        ...
        return status_utils.stylize(Lsp and (spinner[math.floor(luv.hrtime() / 12e7) % #spinner + 1] ..
                                        table.concat(
                {Lsp.title or "", Lsp.message or "", Lsp.percentage and "(" .. Lsp.percentage .. "%)" or ""}, " ")),
            opts)
    end
end
