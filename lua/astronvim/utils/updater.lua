    ...
    local snapshot = vim.tbl_map(function(plugin)
        ...
        if file then
            ...
            if plugin.version then
                ...
                file:write(( -- add hard code for neovim 0.8 support
                plugin[1] == "nvim-treesitter/nvim-treesitter" and
                               'vim.fn.has "nvim-0.9" ~= 1 and "63260da18bf273c76b8e2ea0db84eb901cab49ce" or %q' or
                               "commit = %q"):format(plugin.commit))
            end
...
function M.create_rollback(write)
    ...
    snapshot.remotes = {
        [snapshot.remote] = git.remote_url(snapshot.remote)
    }

    if write == true then
        local file = assert(io.open(astronvim.updater.rollback_file, "w"))
        file:write("return " .. vim.inspect(snapshot, {
            newline = " ",
            indent = ""
        }))
        file:close()
...
function M.update_available(opts)
    ...
    for remote, entry in pairs(opts.remotes and opts.remotes or {}) do
        ...
        if not current_url then
            ...
            check_needed = true
        elseif current_url ~= url and confirm_prompt(
...
function M.update(opts)
    ...
    not opts.skip_prompts and
        not confirm_prompt(
            ("Update available to %s\nUpdating requires a restart, continue?"):format(available_update.version or
                                                                                          available_update.target)) then
        ...
        if #breaking > 0 and not opts.skip_prompts and
            not confirm_prompt(
        ...
        if not updated and not opts.skip_prompts and
            not confirm_prompt(
