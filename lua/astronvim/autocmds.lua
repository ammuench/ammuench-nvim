...
autocmd("BufWritePre", {
    ...
    callback = function(args)
        if args.match:match("^%w%w+:" .. (vim.fn.has "win32" == 0 and "//" or "\\\\")) then
            return
        end
...
local terminal_settings_group = augroup("terminal_settings", {
    clear = true
})
    ...
    autocmd({"User", "BufWinEnter"}, {
        ...
        callback = function(args)
            if ((args.event == "User" and args.file == "AlphaReady") or
                (args.event == "BufWinEnter" and vim.api.nvim_get_option_value("filetype", {
                    buf = args.buf
            ...
            elseif vim.g.before_alpha and args.event == "BufWinEnter" and vim.api.nvim_get_option_value("buftype", {
                buf = args.buf
            }) ~= "nofile" then
                vim.opt.laststatus, vim.opt.showtabline, vim.opt.cmdheight = vim.g.before_alpha.laststatus,
                    vim.g.before_alpha.showtabline, vim.g.before_alpha.cmdheight
...
autocmd("ColorScheme", {
    ...
    group = augroup("astronvim_highlights", {
        clear = true
    }),
    callback = function()
...
autocmd("FileType", {
    ...
    group = augroup("editorconfig_filetype", {
        clear = true
    }),
    callback = function(args)
...
autocmd({"BufReadPost", "BufNewFile", "BufWritePost"}, {
    ...
    callback = function(args)
        ...
        if not (current_file == "" or vim.api.nvim_get_option_value("buftype", {
            buf = args.buf
        }) == "nofile") then
            astroevent "File"
            if require("astronvim.utils.git").file_worktree() or
                utils.cmd({"git", "-C", vim.fn.fnamemodify(current_file, ":p:h"), "rev-parse"}, false) then
                astroevent "GitFile"
